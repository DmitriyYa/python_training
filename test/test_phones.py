...
def test_phones_on_homepage(app):
    ...
    assert some_user_from_home_page.first_name == some_user_from_edit_page.first_name

    assert some_user_from_home_page.last_name == some_user_from_edit_page.last_name

    assert some_user_from_home_page.middle_name == some_user_from_edit_page.middle_name

    assert some_user_from_home_page.all_phones_from_home_page == merg_phones_like_on_home_page(
        some_user_from_edit_page)

    assert some_user_from_home_page.all_emails_from_home_page == merg_emails_like_on_home_page(
        some_user_from_edit_page)
...
def test_phones_on_view_page(app):
    user_from_view_page = app.user.get_user_info_by_index_from_view_page(0)
    user_from_edit_page = app.user.get_user_info_by_index_from_edit_page(0)
